...
// Storage: Supabase when SUPABASE_URL + SUPABASE_ANON_KEY are set (one `trips` row per room, the trip as jsonb),
...
export async function getTrip(code: string): Promise<Trip | null> {
  ...
  const { data } = await sb.from("trips").select("data").eq("code", c).maybeSingle();
...
export async function saveTrip(t: Trip): Promise<void> {
  ...
  const { error } = await sb.from("trips").upsert({ code: t.code, data: t, updated_at: new Date().toISOString() });
