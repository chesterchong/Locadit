...
Send transparent PNG cut-outs (background removed), roughly twice the listed size for sharp rendering. Name each file after the slot id and drop it in `public/collage/`. Polaroid slots take plain photos (the white frame is drawn by CSS). The phone video is one portrait clip (9:19.5).
